  ...
  return (
    <div className="flex items-center gap-1 shrink-0">
      ...
      <button
        type="button"
        onClick={() => {
          if (window.confirm('¿Eliminar esta entrada del perfil?')) {
            fetch(`/api/profiles/${profileId}/sections/${sectionKey}/${entry.id}`, { method: 'DELETE' })
              .then(() => {
      ...
      {entry.verified && evidence.length > 0 ? (
        ...
        <span
          className="p-1.5 text-green-600 dark:text-green-400 cursor-default"
          title="Verificado manualmente (sin documento adjunto)"
          aria-label="Verificado sin documento"
        >
          <ManualCheckIcon />
        </span>
      ) : (
...
function ManualCheckIcon() {
  return (
    <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      <title>Verificado manualmente — sin documento adjunto</title>
